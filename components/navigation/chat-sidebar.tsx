...
export function ChatSidebar() {
  ...
  return (
    <div className="h-full w-64 bg-black/80 border-r border-gray-500/30 flex flex-col">
      {/* Search and new chat */}
          ...
          <div key={group.id} className="py-2">
            ...
            {group.chats.map((chat) => (
              <Link
                ...
                href={`/chat/${chat.id}`}
                className={cn(
                  "flex items-center px-3 py-2 gap-2 text-sm hover:bg-gray-800/30 relative group",
                  pathname === `/chat/${chat.id}` ? "bg-cyan-900/20 text-cyan-400" : "text-gray-300",
                )}
              >
                {chat.avatar ? (
                  ...
                  <div
                    className={cn(
                      ...
                      pathname === `/chat/${chat.id}` ? "bg-cyan-900/50 text-cyan-300" : "bg-gray-800 text-gray-300",
                    )}
                  >
                    {chat.name.charAt(0)}
                  </div>
